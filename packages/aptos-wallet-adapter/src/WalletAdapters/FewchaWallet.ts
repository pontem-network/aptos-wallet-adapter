...
export class FewchaWalletAdapter extends BaseWalletAdapter {
  ...
  async signTransaction(
    ...
    try {
      ...
      let tx;
      let response;
      if (payload instanceof TransactionPayloadScriptFunction) {
        tx = await provider.generateRawTransaction(wallet.address || '', payload);
      } else if (payload instanceof TransactionScriptFunctionPayloadOfRawArguments) {
      ...
      if (
        payload instanceof TransactionPayloadScriptFunction ||
        payload instanceof TransactionScriptFunctionPayloadOfRawArguments
      ) {
